...
title: How to version upgradeable contracts
...
In our [recent guide on smart contract versioning](https://www.enscribe.xyz/blog/smart-contract-versioning), we explored how smart contracts can be versioned & suggested a few ideas on how to integrate contract versioning in your existing deployment pipelines. This post is a sequel explaining how similar ideas can be applied when you have upgradeable contracts.
...
Let’s first see how the [TransparentUpgradeableProxy proxy contract from OpenZeppelin](https://github.com/OpenZeppelin/openzeppelin-contracts/blob/master/contracts/proxy/transparent/TransparentUpgradeableProxy.sol) that uses the transparent upgradeable proxy pattern looks like.
...
We can now deploy both these contracts & point the proxy contract to the implementation by using a number of ways:
...
Should you want to update the implementation, you can simply deploy & name it as `v2.app.mydomain.eth` & upgrade the proxy to point to this new version.

...
{/* ![Sign in with Ethereum and self-managed naming cover image](./cover.jpg) */}
...
Behind the scene we have used [Clerk's](https://clerk.com/docs/guides/configure/auth-strategies/sign-up-sign-in-options#web3-authentication) inbuilt support for SIWE.

## Self-managed naming

Wallet accounts can pick a new manager mode for a namespace: **Self Managed**.

A quick recap on what a manager mode does. When you set up a namespace, you choose who holds the manager rights for the ENS name, and therefore who executes naming changes:
...
- **Self Managed.** Your wallet stays both the owner and the manager. Nothing is transferred to Enscribe or a Safe.

In self-managed mode, you sign every change yourself. Creating a subname, deleting one, or updating records and metadata each happen as transactions from your own wallet. This suits people who want to keep full custody of their name.

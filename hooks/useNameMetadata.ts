...
import { COIN_TYPE_MAPPING } from '@/lib/nameHistory'
...
interface UseNameMetadataProps {
  initialName?: string
}
...
export function useNameMetadata({ initialName }: UseNameMetadataProps) {
  ...
  const refreshMetadataAndHistoryAfterEdit = async (name: string) => {
    const previousLatestHistoryId = latestRawMetadataHistoryId
  ...
  const getAvailableMetadataByCategory = (
    metadataList: Array<{ key: string; label: string; placeholder: string }>,
  ) => {
    const existingKeys = new Set(editingRecords.map((r) => r.key))
